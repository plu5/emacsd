...
(defun faire-tache ()
  ...
  (when (looking-at "^- \\(.+@.*\\)")
    (let* ((s (match-string-no-properties 1))
           ...
           (d (format-time-string "%-e"))  ; jour sans padding
      ...
      (when (< (string-to-number h) 7)
        ...
        (when (equal d "0")
          (let ((m (1- (string-to-number (format-time-string "%m"))))
                (y (string-to-number (format-time-string "%Y"))))
            (when (= m 0) (setq m 12) (setq y (1- y)))
            (setq d (number-to-string (calendar-last-day-of-month m y))))))
      (setq j (concat "^- _[A-Za-z]*_ (" d "):"))
      (while (and (not (looking-at-p j))
                  (zerop (forward-line -1)))
        t)
      (if (bobp)
          (message "jour pas trouvé : %s" j)
        (forward-line)       ; entrer dans la sous-liste de la semaine
